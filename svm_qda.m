%% SVM vs QDA
% How much data does the SVM and QDA need in order to generalize.
...
sXb = (randn(10000,2)*3) * [1 2; 3 2] * [cos(theta2) -sin(theta2); sin(theta2) cos(theta2)] + 15;
separable.X = [sXa; sXb]; % combine into single dataset
separable.Y = [ones(10000,1);zeros(10000,1)]; % set labels
s = separable; % for convenience
s.X = s.X(shuffle,:); % shuffle
s.Y = s.Y(shuffle,:);

nsXa = (randn(10000,2)*3 + 2) * [2 1; 1 1] * [cos(theta) -sin(theta); sin(theta) cos(theta)];
nsXb = (randn(10000,2)*3 + 2) * [2 1; 1 1] * [cos(theta) -sin(theta); sin(theta) cos(theta)] + 5;
...
for i = 1:length(N)
    ...
    % train svm
    smodel = fitcsvm(sX, sY, 'boxconstraint', C, 'KernelFunction', 'polynomial');
    nsmodel = fitcsvm(nsX, nsY, 'boxconstraint', C, 'KernelFunction', 'polynomial');
    ...
    sparams = TrainQDA(sX,sY);
    nsparams = TrainQDA(nsX, nsY);
    
    % test qda
    
    sp = TestQDA(sparams, s.test.X, [1,0]);
    nsp = TestQDA(nsparams, ns.test.X, [1,0]);
    
    % calculate percent correct
    qda_spc(i) = sum(abs(sp - s.test.Y)) / length(sp);
    qda_nspc(i) = sum(abs(nsp - ns.test.Y)) / length(nsp);
...
figure
plot((1:10),[svm_spc'; svm_nspc'; qda_spc'; qda_nspc'])
legend('svm s','svm ns','qda s','qda ns')
